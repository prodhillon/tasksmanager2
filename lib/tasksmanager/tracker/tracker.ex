defmodule Tasksmanager.Tracker do
  @moduledoc """
  The Tracker context.
  """
  ...
  def timeblocks_map_for(user_id) do
    query = from(t in Task, select: t.id , where: t.user_id == ^user_id)
    tasks = Repo.all(query)
    timeblocks = from(t in TimeBlock, where: t.task_id in ^tasks, where: is_nil(t.endtime))
      Repo.all(timeblocks)
